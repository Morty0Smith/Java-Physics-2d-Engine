...
public class PhysicsPlayground extends Application {
  ...
  private BoxCollider[] cursorCollidingColliders = new BoxCollider[1];
  SceneObject dragObject = null;
  BoxCollider cursorCollider;
  private int gridSize = 30;
  private boolean isInSceneEditMode = false;
  boolean arrowCollided = false;
  ...
  public void start(Stage primaryStage) { 
    ...
    imageView1.setOnMouseDragged((mouseEvent) -> {mouseMoved(mouseEvent);});
    imageView1.setOnMouseReleased((mouseEvent) -> {mouseReleased(mouseEvent);});
    imageView1.setOnMousePressed((mouseEvent) -> {mousePressed(mouseEvent);});
    imageView1.setOnMouseClicked((mouseEvent) -> {mouseClicked(mouseEvent);});
    ...
    AddRectangle(100,200,400,150,90,true,true,Color.RED,this);
    //AddRectangle(100,200,200,0,90,true,true,Color.RED,this);
    AddRectangle(700,40,350,350,90,false, true,Color.WHITE,this);
    AddRectangle(700,40,350,400,90,false, true,Color.WHITE,this);
  ...
  Runnable sceneUpdate = new Runnable() {
    public void run() {
      ...
      cursorCollidingColliders = cursorCollider.calculateBoxCollisions(allBoxColliders.length - 1);
      if (dragObject != null) {
        dragObject.setPosition(currentMousePosition[0], currentMousePosition[1]);
      } // end of if
      UpdateFrame(true);
    }
  };
  ...
  public void mouseMoved(MouseEvent event) {
    ...
    if (arrowCollided) {
      for (int i = 0; i < sceneObjects.size(); i++) {
        ArrowTransformEditor arrowsEditor = sceneObjects.get(i).getArrowTransformEditor(); 
        arrowsEditor.handleDrag(currentMousePosition[0], currentMousePosition[1]);
      }
    } // end of if
  }
  ...
  public void mousePressed(MouseEvent event) {
    for (int i = 0; i < sceneObjects.size(); i++) {
      ArrowTransformEditor arrowsEditor = sceneObjects.get(i).getArrowTransformEditor(); 
      if (arrowsEditor.getVisibility()) {
        arrowCollided= arrowsEditor.checkArrowCollisions(cursorCollider);
        System.out.println(arrowCollided);
      } // end of if
    }
  }
  ...
  public void mouseClicked(MouseEvent event) {
    ...
    if (!arrowCollided && cursorCollidingColliders.length > 0 && cursorCollidingColliders[0] != null) {
      for (int i = 0; i < sceneObjects.size(); i++) {
        sceneObjects.get(i).getArrowTransformEditor().setVisible(false);
      }
      cursorCollidingColliders[0].getParentObject().getSceneObject().getArrowTransformEditor().setVisible(true);
    } // end of if
  }
  
  private void renderArrow(Arrow arrow) {
    int xPosition = (int)arrow.getX();
    int yPosition = (int)arrow.getY();
    int startX = xPosition - arrow.getArrowImage().getLenght() / 2;
    int startY = yPosition - arrow.getArrowImage().getHeight() / 2;
    int length = arrow.getArrowImage().getLenght();
    int height = arrow.getArrowImage().getHeight();
    for (int x = 0; x < length; x++) {
      for (int y = 0; y < height; y++) {
        if ((startX + x) >= 0 && (startX + x) < stageImage.length && (startY + y) >= 0 && (startY + y) < stageImage[0].length) {
          if (arrow.getArrowImage().getTransparencyMatrix()[x][y]) {
            stageImage[startX + x][startY + y] = arrow.getArrowImage().getImage()[x][y];
          } // end of if
        } // end of if
      }
    }
  ...
  private void renderVisibleArrows() {
    for (int i = 0; i < sceneObjects.size(); i++) {
      ArrowTransformEditor arrowsEditor = sceneObjects.get(i).getArrowTransformEditor(); 
      if (arrowsEditor.getVisibility()) {
        Arrow[] arrows = arrowsEditor.getArrows();
    ...
    for (int x = 0; x < stageImage.length; x++) {
      for (int y = 0; y < stageImage[0].length; y++) {
        if (x%gridSize == 0 || y%gridSize == 0) {
          stageImage[x][y] = Color.WHITE;
        } // end of if
        else {
          stageImage[x][y] = Color.rgb(20,20,20);
        } // end of if-else
      }
    }
  } 
  ...
  private void UpdateFrame(boolean isSceneUpdate) {
    renderBackground();
    ...
    currentFrame++;
    lastImage = Image.copyColors(stageImage);
  }
  ...
  private void setupObjects() {
    ...
    for (int i = 0; i < sceneObjects.size(); i++) {
      ...
      editObject.getDisplayImage().setRotation(sceneEditObject.getRotation()-90);
      editObject.setSceneObject(sceneEditObject);
      if (editObject.getBoxCollider() != null) {
        editObject.getBoxCollider().setRotation(sceneEditObject.getRotation());
        editObject.getBoxCollider().setPosition(editObject.getX(),editObject.getY());
        editObject.getBoxCollider().setParent(editObject);
      } // end of if
  ...
  private void AddRectangle(int sizeX, int sizeY, int posX, int posY, double rotation, boolean hasRigigdbody, boolean hasCollider, Color baseColor, PhysicsPlayground stage) {
    ...
    if (hasCollider) {
      BoxCollider bc = new BoxCollider(sizeX,sizeY,objects.get(objects.size()- 1).getX(),objects.get(objects.size()- 1).getY(), objects.get(objects.size()- 1), this);
  ...
  static boolean[][] deepCopyBooleans(boolean[][] originalArray) {
    boolean[][] copyToArray = new boolean[originalArray.length][originalArray[0].length];
    for(int x = 0; x < originalArray.length; x++)
    {
      for(int y = 0; y < originalArray[0].length; y++)
      {
        copyToArray[x][y] = originalArray[x][y];
      } 
    } 
    return copyToArray;
  }
  ...
  private void bPlay_Action(Event evt) {
    ...
    if (bPlay.getText().equals("Play")) {
      ...
      animation = new AnimationTimer() {
        ...
        public void handle(long now) {
          if (currentFrame > lastRenderedFrame) {
            ...
            ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor();
            executor.schedule(gameUpdate, 0, TimeUnit.MILLISECONDS);
            imageView1.setImage(stageImageFile);
      ...
      isInSceneEditMode = true;
      setupObjects();
      //Render first frame (otherwise, rendering would never start
      Thread t = new Thread(gameUpdate);
      t.start();
      RenderWhite();
      cursorCollider = new BoxCollider(20,20,0,0,objects.get(0),this);
  ...
  public void RenderWhite() {
    ...
    lastImage = Image.copyColors(colorToRender);
  }
  ...
  public void bTester_Action(Event evt) {
    // TODO hier Quelltext einfuegen
    for (int i = 0; i < allBoxColliders.length; i++) {
      if (allBoxColliders[i].getParentObject() != null) {
